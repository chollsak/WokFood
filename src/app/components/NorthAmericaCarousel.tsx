...
const NorthAmericaCarousel: React.FC = () => {
  const [northAmericaFoods, setNorthAmericaFoods] = useState<Food[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    const fetchNorthAmericaData = () => {
      fetch('/data/data.json') // Adjust the path based on your public folder structure
        ...
        .then((data: RootObject) => {
          setNorthAmericaFoods(data.data['north-america'].Foods); // Access the North America food data
        })
        .catch((err: any) => {
  ...
  return (
    <div style={{ width: '100%', margin: '0 auto' }}>
      <Carousel
        ...
        keyBoardControl
      >
        {northAmericaFoods.map((food) => (
          <CardFood 
            key={food.FoodId} 
            image={food.FoodImageUrl} 
            name={food.FoodName} 
            region={'north-america'} // Assuming the country description is used here
...
export default NorthAmericaCarousel;
