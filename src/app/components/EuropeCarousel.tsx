...
const EuropeCarousel: React.FC = () => {
  const [europeFoods, setEuropeFoods] = useState<Food[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    const fetchEuropeData = () => {
      fetch('/data/data.json') // Adjust the path based on your public folder structure
        ...
        .then((data: RootObject) => {
          setEuropeFoods(data.data.europe.Foods); // Access the Europe food data
        })
        .catch((err: any) => {
  ...
  return (
    <div style={{ width: '100%', margin: '0 auto' }}>
      <Carousel
        ...
        keyBoardControl
      >
        {europeFoods.map((food) => (
          <CardFood 
            key={food.FoodId} 
            image={food.FoodImageUrl} 
            name={food.FoodName} 
            region={'europe'} // Assuming the country description is used here
