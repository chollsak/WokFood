...
const AfricaCarousel: React.FC = () => {
  const [asiaFoods, setAsiaFoods] = useState<Food[]>([]);
    ...
    const fetchAsiaData = () => {
      fetch('/data/data.json') // Adjust the path based on your public folder structure
        ...
        .then((data: RootObject) => {
          setAsiaFoods(data.data.africa.Foods); // Access the Africa food data
        })
        .catch((err: any) => {
      ...
      >
        {asiaFoods.map((food) => (
          <CardFood 
            ...
            name={food.FoodName} 
            region={'africa'} // Assuming the country description is used here
