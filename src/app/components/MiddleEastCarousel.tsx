...
const MiddleEastCarousel: React.FC = () => {
  const [middleEastFoods, setMiddleEastFoods] = useState<Food[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    const fetchMiddleEastData = () => {
      fetch('/data/data.json') // Adjust the path based on your public folder structure
        ...
        .then((data: RootObject) => {
          setMiddleEastFoods(data.data['middle-east'].Foods); // Access the Middle East food data
        })
        .catch((err: any) => {
  ...
  return (
    <div style={{ width: '100%', margin: '0 auto' }}>
      <Carousel
        ...
        keyBoardControl
      >
        {middleEastFoods.map((food) => (
          <CardFood 
            key={food.FoodId} 
            image={food.FoodImageUrl} 
            name={food.FoodName} 
            region={'middle-east'} // Assuming the country description is used here
...
export default MiddleEastCarousel;
