...
async function fetchOceaniaData(): Promise<RegionData> {
  const filePath = path.join(process.cwd(), 'public', 'data', 'data.json');
  const jsonData = await fs.readFile(filePath, 'utf-8');
  const data = JSON.parse(jsonData);
  
  console.log("Fetched data:", data); // Log the entire data
  return data.data.oceania; // Corrected access to nested asia data
}

const SearchResult = async () => {
  const regionData = await fetchOceaniaData();
  ...
  return (
    <Box>
      <CountryBanner
        ...
        bannerImage="https://www.cuddlynest.com/blog/wp-content/uploads/2021/01/aerial-view-palau-makal-island.jpg"
      />
