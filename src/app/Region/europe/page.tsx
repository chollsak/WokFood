...
async function fetchEuropeData(): Promise<RegionData> {
  const filePath = path.join(process.cwd(), 'public', 'data', 'data.json');
  const jsonData = await fs.readFile(filePath, 'utf-8');
  const data = JSON.parse(jsonData);
  
  console.log("Fetched data:", data); // Log the entire data
  return data.data.europe; // Corrected access to nested asia data
}

const SearchResult = async () => {
  const regionData = await fetchEuropeData();
  ...
  return (
    <Box>
      <CountryBanner
        countryName={'EUROPE'}
        bannerImage="https://media.nomadicmatt.com/stayineurope005.jpg"
